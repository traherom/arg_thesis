...
\begin{table}
\caption{Basic tests, invalid packet loss}
...
\begin{figure}
\caption{Hop rate tests, \ac{UDP} and \ac{TCP} traffic between \ac{ARG} networks packet loss}
...
\begin{figure}
\caption{Hop rate tests, scaled view of traffic between \ac{ARG} networks}
...
\caption{With 5 millisecond \ac{ARP}}
\end{subfigure}
\begin{subfigure}[t]{.6\linewidth}
\centering
\includegraphics[width=1.0\linewidth]{triple_arp_issue_no_arp}
\caption{With insignificant \ac{ARP} (not shown)}
\end{subfigure}
\end{figure}
...
\begin{figure}
\caption{Hop rate tests, \ac{TCP} traffic between \ac{ARG} networks packet loss} 
...
\begin{figure}
\caption{Hop rate tests, externally-bound packet loss}
