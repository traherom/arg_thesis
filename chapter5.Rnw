...
@
\caption{Valid packet loss, basic tests raw data}
...
\par \ac{ARG} is stable in the face of bad traffic. Long-running fuzz tests do not cause \ac{ARG} to crash, nor do they permanently break traffic flow. However, malformed traffic does appear to have an impact on valid packet loss, based on a visual inspection. The cause of this needs exploration to determine the correct fix.
\end{enumerate}
